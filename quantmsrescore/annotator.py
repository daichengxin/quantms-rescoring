...
class Annotator:
    ...
    def _run_deeplc_annotation(self) -> None:
        ...
        try:
            if self._skip_deeplc_retrain:
                ...
                deeplc_annotator = self._select_best_deeplc_model()
    ...
    def _select_best_deeplc_model(self) -> DeepLCAnnotator:
        """
        Select the best DeepLC model by comparing retrained vs. pretrained performance.

        Returns
        -------
        DeepLCAnnotator
            Best performing DeepLC annotator.
        """
        # Create a copy of PSMs for comparison
        ...
        if mae_trained < mae_pretrained:
            logging.info(
                f"Retrained DeepLC model has lower MAE ({mae_trained:.4f} vs {mae_pretrained:.4f}), using it: {retrained_annotator.selected_model}"
            )
            return retrained_annotator
        else:
            logging.info(
                f"Pretrained DeepLC model has lower/equal MAE ({mae_pretrained:.4f} vs {mae_trained:.4f}), using it: {pretrained_annotator.selected_model}"
            )
            return pretrained_annotator
